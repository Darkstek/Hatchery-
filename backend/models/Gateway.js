...
// GET /api/gateway/settings — načte teplotní rozsah (chráněno JWT)
router.get("/settings", jwtAuth, async (req, res) => {
  try {
    const { gatewayId } = req.query;
    if (!gatewayId) {
      return res.status(400).json({ error: "Chybí gatewayId" });
    }
    const gateway = await Gateway.findOne({ gatewayId });
    if (!gateway) {
      return res.status(404).json({ error: "Gateway nenalezena" });
    }
    res.json({ tempMin: gateway.tempMin, tempMax: gateway.tempMax });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "Chyba serveru" });
  }
